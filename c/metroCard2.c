...
arr_integer solution(int lastNumberOfDays) {
   ...
   if (lastNumberOfDays<=30) {	   
        a.size=1; 
		a.arr=malloc(a.size * sizeof(*a.arr));
        a.arr[0]=31;
		return(a);
    }
    else {
        b.size=3;
		b.arr=malloc(b.size * sizeof(*b.arr));
        b.arr[0]=28;
        b.arr[1]=30;
        b.arr[2]=31;
		return(b);
...
int main()
{
	arr_integer alloc_arr_integer;
	printf("myarray for %d = \n",30);
	alloc_arr_integer.size=1;
	alloc_arr_integer=solution(30);
	printArray(alloc_arr_integer.arr,1);
	
	alloc_arr_integer.size=3;
	printf("myarray for %d = \n",31);
	alloc_arr_integer=solution(31);
	printArray(alloc_arr_integer.arr,3);
